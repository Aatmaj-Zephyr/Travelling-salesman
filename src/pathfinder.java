...
public class pathfinder {
  ...
  public citylist sendpath() { // return the citylist path which stores the shortest path
    return path;
  }

  public void printpathname() { // print the shortest path.
    path.print("-");
  }

  public double distance() { // Return the shortest distance.
    return distance;
  }
  ...
  private double FindPath(city START, city END, double min) {
    ...
    // If the temp citylist's last element is our destination END.....
    {
      ...
      if (dist
          < min) { // If the sum is the minimum till now, then the value of citylist path equals
                   // that of temp.
        path.set(temp);
      }
    ...
    {
      for (int i = 0; i < START.neighbours.size(); i++) { // For every neighbour of START,.......
