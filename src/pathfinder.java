public class pathfinder {
  ...
  pathfinder(
    ...
    distance = findPath(START, END); // find the shortest distance
  }
  ...
  private double findPath(city START, city END){
    return findPath(START, END, 1000.00);
    //1000.00 means a big number which is greater thsn any of the shortest paths possible. This is used for recursion.
  }
  private double findPath(city START, city END, double min) {
    ...
    {
      double dist = reachedDestinationReturnPathFound(min, END);
      return dist; // return the shortest distance

    } else // Else....
    {
      double dist = calculatePathRecursively(min, START, END);
      return dist;
      // now no neighbours are left.
    }
  }
  private double reachedDestinationReturnPathFound(double min, city END){
    double dist = tempCityList.getPathLength();
      ...
      if (dist < min) { // If the sum is the minimum till now, then the value of citylist path equals
                        // that of temp.
        shortestPath.createNewCopyFromCitylist(tempCityList);
      }
  ...
  private double calculatePathRecursively(double min,city START,city END){
